...
app.post('/login', function(req, res, next) {
  passport.authenticate('local', function(err, user, info) {
    ...
    if (!user) {
      return res.redirect(
        '/login' + dest ? '?to=' + encodeURIComponent(dest) : ''
      );
    }
...
function addDbRoute(app, formattedDb) {
  ...
  d.run(() => {
    app.use(formattedDb.roPath, mongoExpress(mongoConfig(formattedDb.url, true)));
    app.use(formattedDb.edPath, mongoExpress(mongoConfig(formattedDb.url, false)));
  });
}
