...
require('./lib/rhmapAuthPassport').init(passport);
...
app.get('/home', function (req, res) {
  getFormattedDbs().then(databases => {
    res.render('home', { databases: databases, isLocal: config.isLocal()});
  });
});
...
getFormattedDbs().then(databases => {
  ...
  if (databases.length === 0) {
    console.log('WARNING: No databases were in mongo, so this app is pretty useless right now...');
  }

  databases.forEach(db => {
    console.log('Loaded database: ', JSON.stringify({ name: db.name, path: db.path }));
